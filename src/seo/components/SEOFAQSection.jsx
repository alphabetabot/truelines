...
export default function SEOFAQSection({ title = 'Frequently asked questions', faqs = [], jsonLd = false }) {
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl font-black mb-4" style={{ color: 'var(--text-primary)' }}>{title}</h2>
      <div className="space-y-3">
        {faqs.map(({ question, answer }) => (
          ...
          >
            <summary className="font-semibold text-sm cursor-pointer list-none flex justify-between gap-2" style={{ color: 'var(--text-primary)' }}>
              {question}
              <span className="text-[#94a3b8] group-open:rotate-45 transition-transform">+</span>
            </summary>
            <p className="text-sm mt-3 leading-relaxed" style={{ color: 'var(--text-muted)' }}>{answer}</p>
          </details>
        ))}
      </div>
    </section>
